...
namespace netcore.fcimiddleware.fondos.api.Controllers.V1
{
    ...
    public class MonedaController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(CodeErrorException), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<Moneda>> GetByIdMoneda([FromQuery] GetByIdMonedasQuery request)
        {
            var moneda = await _mediator.Send(request);
            return Ok(moneda);
        }

        [HttpGet("list", Name = "GetByDescripcionMoneda")]
        [ProducesResponseType(typeof(PaginationVm<MonedaListVm>), (int)HttpStatusCode.OK)]
        [ProducesResponseType(typeof(CodeErrorException), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<PaginationVm<MonedaListVm>>> GetByDescripcionMoneda([FromQuery] GetByDescripcionMonedasQuery request)
        {
            var moneda = await _mediator.Send(request);
            return Ok(moneda);
        }
